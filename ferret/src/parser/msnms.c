...
static unsigned
matches_nocase(const void *lhs, const void *rhs, unsigned length)
{
	unsigned i;
	for (i=0; i<length; i++) {
...
static void msg_content_type(struct StringReassembler *data, const unsigned char **r_px, unsigned *r_length)
{
	...
	const unsigned char *px = data->the_string;
	unsigned length = data->length;
	unsigned content_type_len = strlen("Content-Type:");
	...
	*r_length = 0;
	...
	for (i=0; i<length; i++) {
		if (px[i] == '\n') {
			...
			if (i+1<length && px[i] == '\r' && px[i+1] == '\n')
				return;
			if (i+content_type_len<length && matches_nocase(px+i, "Content-Type:", content_type_len)) {
				unsigned offset;
				i += content_type_len;
				while (i < length && px[i] != '\n' && isspace(px[i]))
					i++;
	...
	case X_CMD('I', 'R', 'O', '\0'): /* JOI */
		{
			...
			strncpy_s(	sess->layer7.msnreq.username, sizeof(sess->layer7.msnreq.username),
						atom.px+atom.offset, atom.len);
	...
	case X_CMD('U', 'S', 'R', '\0'): /* USR */
		...
		if (!atom_is_number(atom))
			...
			if (!atom_equals_ignorecase(atom, "OK")) {
				...
				strncpy_s(	sess->layer7.msnreq.toname, sizeof(sess->layer7.msnreq.toname),
					username.px+username.offset, username.len);
