...
unsigned is_known_method(const char *method, unsigned method_length)
{
	const char *known_methods[] = {
		"GET",
			0};
	unsigned i;

	for (i=0; known_methods[i]; i++) {
		unsigned len = strlen(known_methods[i]);
		if (len != method_length)
			continue;
		if (memcmp(known_methods[i], method, method_length) == 0)
			return 1;
	}
	return 0;
}
...
void parse_http_request(struct TCPRECORD *sess, struct NetFrame *frame, const unsigned char *px, unsigned length)
{
	...
	case HTTP_METHOD:
		...
		if (offset<length && isspace(px[offset])) {
			if (!is_known_method(req->method, req->method_length))
				FRAMERR(frame, "unknown HTTP method: %.*s\n", req->method_length, req->method);
