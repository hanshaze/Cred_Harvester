...
/**
 * Sift the FERRET record for information HAMSTER is interested in,
 * such as a usernames and machine IDs
 */
...
void vJOTDOWN(struct Ferret *ferret, va_list marker)
{
	...
	for (record_count=0; record_count<MAX_RECORDS; record_count++) {
		...
		bc = bintree_insert(bc, (const char*)name, strlen(name), &is_new_entry);
		bc_vector[record_count*2 + 0] = bc;
		bc = bintree_insert(bc, (const char *)valbuf, vallen, &is_new_entry);
