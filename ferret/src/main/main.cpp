...
void process_live(struct Ferret *ferret, const char *devicename)
{
	...
	while (ferret->interface_search != 0 && ferret->interface_search[0] != 0) {
		 ...
		 */
		{
			...
			channel = beacon_get_channel_from_ssid(ferret, &ferret->interface_search[0], strlen(ferret->interface_search));
			if (channel != 0) {
...
void main_set_wifi_defaults(struct Ferret *ferret, const char *channel)
{
	...
	if (!pcap.is_available) {
		fprintf(stderr, "FAILURE: can't link to pcap library\n");
		exit(1);
	}
	...
	if (desired_interface && desired_level > 0) {
		...
		fprintf(stderr, "ERR: WiFi interface not found, configure manually with '-i'\n");
	}
...
main_args(int argc, char **argv, struct Ferret *ferret)
{
	...
	for (i=1; i<argc; i++) {
		...
		if (arg[1] == '-') {
			...
			} else if (stricmp(arg, "hamster") == 0) {
				ferret_set_parameter(ferret, "hamster.mode", "true", 0);
			} else {
				fprintf(stderr, "%s: unknown option\n", argv[i]);
			}
			continue;
...
for (i=0; i<argc; i++) fprintf(stderr, "[%d] %s\n", i, argv[i]);
	...
	if (ferret->is_live) {
		...
		if (ferret->snarfer.mode == FERRET_SNIFF_UNKNOWN && !ferret->cfg.hamster_mode) {
			ferret_set_parameter(ferret, "sniffer.mode", "most", 0);
		...
		fprintf(stderr, "-- Sniffing on interface \"%s\"\n", ferret->interface_name);
	 ...
	 */
	{
		...
		frame->time_secs = ferret->now + 60*60;
		frame->time_usecs = 0;
